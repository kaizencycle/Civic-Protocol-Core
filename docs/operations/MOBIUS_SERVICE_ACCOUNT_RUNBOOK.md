...
| `503 Identity database write failed` on signup | `DATABASE_URL` not writable or bcrypt/passlib version mismatch | Wire Render disk; pin `bcrypt==4.0.1` with `passlib==1.7.4` (see identity/requirements.txt) |
